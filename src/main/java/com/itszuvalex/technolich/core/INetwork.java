 ...
 *
 * Networks mainly exist to host single-tick update algorithms that are lower complexity than node-based algorithms - see power transfer.
 *      If there are P producers and C consumers -
 *          node by node, each of those P producers would have to check C consumers to see who they can distribute power to, resulting in P*C complexity.
 *          network - knows all P producers and C consumers, by traversing them in some manner, we can visit each node only once, resulting in P+C complexity.
 *      For small networks (2 of each) this is exactly equal.  For larger networks (50 each), we visit 100 instead of 250 times.
 *      For a ridiculous network (150), we visit 300 instead of 22,500.
 *
 *      We also get niceties such as network-wide metric tracking.
 *
 * Networks only track BlockEntities that are in loaded chunks.  When a chunk unloads, we have {@link #removeNodes(Stream)} to prevent recalculating
